...
!function ($) {
	...
	DateRangePicker.prototype = {
		...
		setOptions: function (options, callback) {
			...
			if (typeof options.disabledDays == 'object')
			...
			if (typeof optionsapplyClass == 'string')
		...
		updateInputText: function () {
			if (this.element.is('input') && !this.singleDatePicker) {
				...
				this.element.val(this.startDate.format(this.format)).change();
			}
		...
		renderCalendar: function (calendar, selected, minDate, maxDate) {
			...
			for (var row = 0; row < 6; row++) {
				...
				for (var col = 0; col < 7; col++) {
					...
					if ((minDate && calendar[row][col].isBefore(minDate)) || (maxDate && calendar[row][col].isAfter(maxDate)) || (this.disabledDays && (_.contains(this.disabledDays, calendar[row][col].day()) ))) {
						cname = ' off disabled ';
